...
public class UserServiceImpl  implements UserService {
    ...
    public int saveUserCount(Integer userId) {
        ...
        if (limitNum == null) {
            //第一次调用放入redis中设置为0
            stringRedisTemplate.opsForValue().set(limitKey, "0", 120, TimeUnit.SECONDS);
        } else {
            //不是第一次调用每次+1
            limit = Integer.parseInt(limitNum) + 1;
            stringRedisTemplate.opsForValue().set(limitKey, String.valueOf(limit), 120, TimeUnit.SECONDS);
        }
        return limit;//返回调用次数
    }
